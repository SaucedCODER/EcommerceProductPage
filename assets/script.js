 ...
 const iquantity = document.querySelector("[data-quantity-val]");
  ...
  if (iquantity.innerText === "0") {
   alert("Warning: Please put some quantity!");
    ...
    const total = 250.00 * parseInt(iquantity.innerText);
    
    cart_item_el.innerHTML = `
    <img src="./images/image-product-1.jpg" alt="pr-img" />
    ...
    <img data-del-btn src="./images/icon-delete.svg" alt="del" />
      `;
...
slider_btns.forEach(btn=>{
  btn.addEventListener("click",()=>{
    ...
    let indexofactiveimg = [...s_images_container.children].indexOf(activeimg) + offset;

    if (indexofactiveimg < 0) indexofactiveimg = s_images_container.children.length - 1;
    if (indexofactiveimg >= s_images_container.children.length) indexofactiveimg = 0;

    s_images_container.children
    //adding dataset to the img
    [indexofactiveimg].dataset.activePr = true;
